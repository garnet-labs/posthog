...
def wait_for_url(url: str, *, timeout: int = 1800, interval: int = 10) -> bool:
...
def init_cache_from_master() -> None:
    ...
    try:
        ...
        if not wait_for_url(f"http://localhost:{init_port}/_health"):
...
def _prompt_migration_choice(stale_info: str) -> str:
    ...
    while True:
        choice = input("Choice [c/f/q]: ").strip().lower()
        if choice in ("c", "f", "q"):
            return choice
...
def cmd_create(branch: str, base_branch: str = "master") -> None:
    ...
    if not wait_for_url(f"http://localhost:{port}/_health"):
        error("PostHog did not start after 1800s. Something is likely broken.")
...
def cmd_shell(branch: str) -> None:
    ...
    # exec replaces this process so the TTY is passed through cleanly
    os.execvp("docker", [
