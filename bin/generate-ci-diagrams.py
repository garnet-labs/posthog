...
PUBLISHED_DIR = REPO_ROOT / "docs" / "published" / "handbook" / "engineering"
...
def generate_published_page(workflows: list[WorkflowInfo], mermaid_by_name: dict[str, str]) -> str:
    """Generate a single combined page for the published handbook."""
    legend_rows = "\n".join(f"| {shape} | {color} | {meaning} |" for shape, color, meaning in LEGEND_ROWS)

    sections: list[str] = []
    for wf in sorted(workflows, key=lambda w: w.filename):
        triggers_str = ", ".join(f"`{t}`" for t in wf.triggers)
        mermaid = mermaid_by_name[wf.filename]
...
def main(argv: list[str] | None = None) -> int:
    ...
    for filename in requested_workflows:
        ...
        mermaid_by_name[wf.filename] = mermaid
