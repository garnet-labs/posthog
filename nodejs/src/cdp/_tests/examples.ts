...
export const HOG_EXAMPLES: Record<string, Pick<HogFunctionType, 'hog' | 'bytecode' | 'type'>> = {
    ...
    simple_send_push_notification: {
        type: 'destination',
        hog: "let res := sendPushNotification({\n    'integrationId': inputs.integrationId,\n    'distinctId': inputs.distinctId,\n    'payload': {\n        'title': inputs.title,\n        'body': inputs.body\n    }\n})\n\nprint('Push response:', res)",
        bytecode: [
            '_H',
            1,
            32,
...
export const HOG_INPUTS_EXAMPLES: Record<string, Pick<HogFunctionType, 'inputs' | 'inputs_schema'>> = {
    ...
    simple_send_push_notification: {
        inputs_schema: [
            ...
            { key: 'distinctId', type: 'string', label: 'Distinct ID', secret: false, required: true },
            { key: 'title', type: 'string', label: 'Notification title', secret: false, required: true },
            { key: 'body', type: 'string', label: 'Notification body', secret: false, required: false },
        ],
        inputs: {
            integrationId: { value: 1 },
            distinctId: {
                value: 'test-distinct-id',
                bytecode: ['_h', 32, 'test-distinct-id'],
            },
            title: {
                value: 'Test notification',
                bytecode: ['_h', 32, 'Test notification'],
            },
            body: {
                value: 'Hello from PostHog',
                bytecode: ['_h', 32, 'Hello from PostHog'],
            },
        },
    },
