...
export const HOG_EXAMPLES: Record<string, Pick<HogFunctionType, 'hog' | 'bytecode' | 'type'>> = {
    ...
    simple_send_push_notification: {
        type: 'destination',
        hog: "let res := sendPushNotification(inputs.url, { 'headers': inputs.headers, 'body': inputs.body, 'method': inputs.method });\n\nprint('Push response:', res)",
        bytecode: [
            '_h',
            32,
            'headers',
...
export const HOG_INPUTS_EXAMPLES: Record<string, Pick<HogFunctionType, 'inputs' | 'inputs_schema'>> = {
    ...
    simple_send_push_notification: {
        inputs_schema: [
            { key: 'url', type: 'string', label: 'FCM URL', secret: false, required: true },
            { key: 'body', type: 'json', label: 'JSON body', secret: false, required: true },
            {
                key: 'method',
                type: 'choice',
        ...
        inputs: {
            url: {
                value: 'https://fcm.googleapis.com/v1/projects/test/messages:send',
                bytecode: ['_h', 32, 'https://fcm.googleapis.com/v1/projects/test/messages:send'],
            },
            method: { value: 'POST' },
            headers: { value: {}, bytecode: {} },
            body: { value: {}, bytecode: {} },
        },
    },
