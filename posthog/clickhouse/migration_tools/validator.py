...
def validate_migration(migration_dir: Path, *, strict: bool = False) -> list[ValidationResult]:
    ...
    results.extend(_check_companion_tables(manifest, sql_content))
    ...
    results.extend(_check_drop_statements(sql_content, strict=strict))
    return results
...
def _check_companion_tables(manifest: MigrationManifest, sql_content: str) -> list[ValidationResult]:
