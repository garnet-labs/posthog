...
class TestExtractClickhouseErrorCode:
    def test_tcp_exception_with_code_attribute(self):
        exc = Exception("some error")
        exc.code = 60
        assert extract_clickhouse_error_code(exc) == 60

    def test_http_exception_with_code_in_message(self):
        exc = Exception(
            "HTTPDriver for http://localhost:8123 received ClickHouse error code 60\n"
            " Code: 60. DB::Exception: Table posthog_test.kafka_events_json does not exist."
        )
        assert extract_clickhouse_error_code(exc) == 60

    def test_http_exception_memory_limit(self):
        exc = Exception("Code: 241. DB::Exception: Memory limit exceeded")
        assert extract_clickhouse_error_code(exc) == 241

    def test_no_code_returns_none(self):
        exc = Exception("Connection refused")
        assert extract_clickhouse_error_code(exc) is None

    def test_code_attribute_takes_precedence(self):
        exc = Exception("Code: 999. some message")
        exc.code = 60
        assert extract_clickhouse_error_code(exc) == 60
...
class TestErrorHandlingCompatibility:
    def test_run_clickhouse_statement_in_parallel_ignores_http_unknown_table(self):
        ...
        http_error = Exception(
            "HTTPDriver for http://localhost:8123 received ClickHouse error code 60\n"
            " Code: 60. DB::Exception: Table posthog_test.kafka_events_json "
            "does not exist. (UNKNOWN_TABLE)"
        )

        with patch("posthog.test.base.sync_execute", side_effect=http_error):
            # Should NOT raise — the function should swallow code-60 errors for posthog_test
            run_clickhouse_statement_in_parallel(["SELECT 1"])

    def test_run_clickhouse_statement_in_parallel_raises_non_60_http_errors(self):
        """Non-UNKNOWN_TABLE HTTP errors still propagate."""
        from posthog.test.base import run_clickhouse_statement_in_parallel
    ...
    def test_extract_code_from_http_database_error(self):
        """Error code extraction works for clickhouse_connect DatabaseError format."""
        msg = (
            "HTTPDriver for http://localhost:8123 received ClickHouse error code 60\n"
            " Code: 60. DB::Exception: Table posthog_test.kafka_events_json "
            "does not exist. (UNKNOWN_TABLE) (version 25.8.12.129 (official build))"
        )
        exc = Exception(msg)
        assert extract_clickhouse_error_code(exc) == 60
