...
def _ok_response(data: dict) -> MagicMock:
    """Create a mock response that passes _check_response."""
    response = MagicMock()
    response.ok = True
...
class TestGatewayClientExecute:
    def test_sends_structured_json(self, mock_session: MagicMock):
        mock_session.post.return_value = _ok_response({"data": [["row1"], ["row2"]], "column_types": []})
    ...
    def test_returns_written_rows_for_inserts(self, mock_session: MagicMock):
        mock_session.post.return_value = _ok_response({"data": [], "written_rows": 42})
    ...
    def test_returns_zero_for_zero_row_insert(self, mock_session: MagicMock):
        mock_session.post.return_value = _ok_response({"data": [], "written_rows": 0})

        client = _make_client(mock_session)
        result = client.execute("INSERT INTO events SELECT ... WHERE 1=0")
    ...
    def test_raises_on_http_error(self, mock_session: MagicMock):
        ...
        response.json.return_value = {"error": "ClickHouse returned 502", "error_type": "clickhouse_error"}
        mock_session.post.return_value = response

        client = _make_client(mock_session)
        with pytest.raises(Exception, match="clickhouse_error"):
            client.execute("SELECT 1")

    def test_passes_query_id(self, mock_session: MagicMock):
        mock_session.post.return_value = _ok_response({"data": []})
    ...
    def test_external_tables_raises(self, mock_session: MagicMock):
        client = _make_client(mock_session)
        with pytest.raises(NotImplementedError, match="external_tables"):
            client.execute("SELECT 1", external_tables=[{"name": "t"}])
...
class TestGatewayRouting:
    def test_gateway_enabled_routes_to_gateway(self, settings):
        ...
        # get_client_from_pool passes workload defaults, so the context manager
        # yields a RoutedGatewayClient wrapping the singleton GatewayClient.
        with ctx as client:
            assert isinstance(client, (GatewayClient, RoutedGatewayClient))
