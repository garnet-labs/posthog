...
class LifecycleQueryRunner(AnalyticsQueryRunner[LifecycleQueryResponse]):
    ...
    def _calculate(self) -> LifecycleQueryResponse:
        ...
        res = self.format_results(response)
    ...
    def format_results(self, response: HogQLQueryResponse) -> list[dict[str, Any]]:
        ...
        Each row in response.results is (date_array, count_array, status_string).
        ...
        results = sorted(response.results, key=lambda row: order.get(row[cols["status"]], 5))
