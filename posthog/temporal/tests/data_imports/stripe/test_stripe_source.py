...
def test_validate_credentials():
    ...
    # Mock each resource's list method via v1 namespace
    ...
    with mock.patch("posthog.temporal.data_imports.sources.stripe.stripe.StripeClient", return_value=mock_client):
        ...
        mock_client.v1.balance_transactions.list.assert_called_once_with(params={"limit": 1})
        ...
        mock_client.v1.disputes.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.invoice_items.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.invoices.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.payouts.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.prices.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.products.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.subscriptions.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.refunds.list.assert_called_once_with(params={"limit": 1})
        mock_client.v1.credit_notes.list.assert_called_once_with(params={"limit": 1})
