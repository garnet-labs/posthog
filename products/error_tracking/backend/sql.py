...
def TRUNCATE_ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_SQL():
    return f"TRUNCATE TABLE IF EXISTS {ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE}"
...
ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_ENGINE = lambda: ReplacingMergeTree(
    ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE, ver="version"
)


def ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_SQL():
    return (
        ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_BASE_SQL
        + """
    ORDER BY (team_id, fingerprint)
    ...
    """
        ...
        on_cluster_clause=ON_CLUSTER_CLAUSE(False),
        engine=ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_ENGINE(),
        extra_fields=f"""
    {KAFKA_COLUMNS_WITH_PARTITION}
    , {index_by_kafka_timestamp(ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE)}
    """,
    )
...
WRITABLE_ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_SQL = (
    lambda: ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE_BASE_SQL.format(
        ...
        on_cluster_clause=ON_CLUSTER_CLAUSE(False),
        engine=Distributed(
            data_table=ERROR_TRACKING_FINGERPRINT_ISSUE_STATE_TABLE,
            cluster=settings.CLICKHOUSE_SINGLE_SHARD_CLUSTER,
        ),
        extra_fields=KAFKA_COLUMNS_WITH_PARTITION,
    )
