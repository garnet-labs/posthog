"""Shared SDK version functions used by the SDK Doctor API and health checks.

Provides single-team ClickHouse queries for the API's force_refresh fallback.
The bulk caching is handled by the sdk_outdated health check.
"""
...
def get_and_cache_team_sdk_versions(
    ...
    *,
    ...
    Used by the SDK Doctor API's force_refresh fallback.
    ...
    try:
        ...
        if sdk_versions is not None:
            ...
            redis_client.setex(cache_key, TEAM_SDK_VERSIONS_CACHE_EXPIRY, payload)
