    ...
    return (
        <>
            ...
            <LemonInput
                ...
                onChange={(val) => {
                    onStateChange({ ...state, interval: val || 1 })
                }}
                className="w-14"
            />
            <LemonSelect
            ...
            {state.endType === 'after_count' && (
                <>
                    <LemonInput
                        ...
                        onChange={(val) => {
                            onStateChange({ ...state, endCount: val || 1 })
                        }}
                        className="w-16"
                    />
...
function SchedulePreview({ state, summary, previewOccurrences }: SchedulePreviewProps): JSX.Element {
    return (
            ...
            {previewOccurrences.length > 0 && (
                <div>
                    <div className="text-xs font-semibold text-muted uppercase tracking-wide mb-2">
                        {state.endType !== 'never' ? `${previewOccurrences.length} occurrences` : 'Next occurrences'}
                    </div>
        ...
        <div className="flex flex-col gap-3 w-full">
            ...
            {startsAt && (
                <div className="text-xs text-muted -mt-1">
                    Timezone: {timezone} · Scheduled for: {dayjs(startsAt).format('MMMM D, YYYY [at] h:mm A')}
                </div>
            )}
            ...
            {isRepeating && (
                <>
                    ...
                    <SchedulePreview state={state} summary={summary} previewOccurrences={previewOccurrences} />
                </>
            )}
        </div>
    )
