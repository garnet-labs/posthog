...
export const workflowLogic = kea<workflowLogicType>([
    ...
    listeners(({ actions, values, props }) => ({
        ...
        saveWorkflowSuccess: async ({ originalWorkflow }) => {
            ...
            if (values.pendingSchedule !== false && originalWorkflow.id) {
                try {
                    const existing = values.currentSchedule

                    if (values.pendingSchedule === null) {
                        if (existing?.id) {
                            await api.hogFlows.deleteHogFlowSchedule(originalWorkflow.id, existing.id)
                        }
                    } else if (existing?.id) {
                        await api.hogFlows.updateHogFlowSchedule(
                            originalWorkflow.id,
                            existing.id,
                            values.pendingSchedule
                        )
                    } else {
                        await api.hogFlows.createHogFlowSchedule(originalWorkflow.id, values.pendingSchedule)
                    }
