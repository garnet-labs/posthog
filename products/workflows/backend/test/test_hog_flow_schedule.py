...
class TestProcessDueScheduleTriggers(APIBaseTest):
    ...
    def test_no_batch_job_created_for_schedule_trigger(self, mock_invocation):
        hog_flow, _ = self._create_workflow_with_schedule(
            next_run_at=datetime(2020, 1, 1, tzinfo=UTC),
        )
        response = self._post()
        assert response.status_code == 200

        assert HogFlowBatchJob.objects.filter(hog_flow=hog_flow).count() == 0
