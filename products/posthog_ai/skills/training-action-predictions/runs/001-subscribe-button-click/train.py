"""
train.py — Run 001: Predict "Subscribe button clicked" on PostHog prod (project 2).
...
Usage:
    ...
    export POSTHOG_PROJECT_ID=2
    cd products/posthog_ai/skills/training-action-predictions/runs/001-subscribe-button-click
    flox activate -- python train.py
"""
...
with open(os.path.join(RUN_DIR, "query.sql")) as f:
...
def main() -> None:
    ...
    # ── Balanced sampling ──────────────────────────────────────────────
    # For rare events, take all positives + equal number of negatives.
    # The isotonic calibration in the pipeline adjusts predicted
    # probabilities back to the true base rate.
    pos = df[df["label"] == 1]
    neg = df[df["label"] == 0]
    n_pos = len(pos)
    n_neg_sample = min(len(neg), max(n_pos, 500))  # at least 500 negatives
    ...
    print(f"Balanced sample: {n_pos} positive, {n_neg_sample} negative")
