...
from products.posthog_ai.backend.models import ActionPredictionModel


class TestActionPredictionModelRunAPI(APIBaseTest):
    def setUp(self):
        super().setUp()
        self.action = Action.objects.create(team=self.team, name="Test Action")
        self.prediction_model = ActionPredictionModel.objects.create(
            team=self.team,
            action=self.action,
            lookback_days=30,
    ...
    def _url(self, pk=None):
        base = f"/api/environments/{self.team.id}/action_prediction_model_runs"
        if pk:
            return f"{base}/{pk}/"
        return f"{base}/"
    ...
    def _create_run(self, **overrides):
        defaults = {
            "prediction_model": self.prediction_model.id,
            "is_winning": False,
    ...
    def test_create(self):
        ...
        self.assertEqual(data["prediction_model"], str(self.prediction_model.id))
        self.assertEqual(data["created_by"]["id"], self.user.id)

    def test_list(self):
        self._create_run()
        self._create_run(is_winning=True, model_url="https://s3.amazonaws.com/bucket/model2.pkl")
    ...
    def test_list_ordered_by_newest_first(self):
        resp1 = self._create_run(model_url="https://s3.amazonaws.com/bucket/first.pkl")
        resp2 = self._create_run(model_url="https://s3.amazonaws.com/bucket/second.pkl")
    ...
    def test_list_scoped_to_team(self):
        ...
        response = self.client.get(f"/api/environments/{other_team.id}/action_prediction_model_runs/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
