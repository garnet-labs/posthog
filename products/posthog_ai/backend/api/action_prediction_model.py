...
class ActionPredictionModelSerializer(serializers.ModelSerializer):
    ...
    def get_training_status(self, obj: ActionPredictionModel) -> str | None:
...
class UploadURLResponseSerializer(serializers.Serializer):
    ...
    storage_path = serializers.CharField(help_text="S3 storage path to use as model_url when creating a run.")
...
class ActionPredictionModelViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    queryset = ActionPredictionModel.objects.select_related("action", "created_by", "task_run").all()
    ...
    def upload_url(self, request, pk=None, **kwargs):
        ...
        storage_path = _artifact_storage_path(self.team_id, str(model.id), "model.pkl")
