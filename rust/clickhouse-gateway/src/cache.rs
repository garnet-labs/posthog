...
///
/// Uses a `ConnectionManager` (lazily initialized) to reuse a single
/// multiplexed connection instead of creating one per operation.
...
impl QueryCache {
    ...
    ///
    /// The actual connection is established lazily on first use via
    /// `ConnectionManager`, which handles automatic reconnection.
    ...
    /// Get or lazily initialize the ConnectionManager.
    ///
    /// ConnectionManager multiplexes commands over a single connection and
    /// automatically reconnects on failure, so we only need one for the
        ...
        let conn = self
            ...
            .get_or_try_init(|| async {
                client.get_connection_manager().await.map_err(|e| {
                    warn!(error = %e, "failed to create redis ConnectionManager");
    ...
    pub async fn set(&self, team_id: u64, sql_hash: &str, result: &CachedResult, ttl_seconds: u64) {
        if !self.enabled {
            return;
        }
