...
mod tests {
    ...
    fn historical_context(historical_migration: bool) -> Context {
        let mut ctx = test_utils::test_context();
        ctx.historical_migration = historical_migration;
        ctx
    }
    ...
    fn historical_batch_flag_reroutes_all_events() {
        ...
        let ctx = historical_context(true);
        let mut events = events_map(vec![
            wrapped_event("$pageview", "user-1"),
            wrapped_event("$identify", "user-2"),
        ]);
    ...
    fn historical_timestamp_reroutes_old_event() {
        ...
        let ctx = historical_context(false);
