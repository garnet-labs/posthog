...
export function tagName(): boolean {
    return true
}
...
function* search(input: Element, config: Options, rootDocument: Element | Document | ShadowRoot): Generator<Knot[]> {
    const stack: Knot[][] = []
    let paths: Knot[][] = []
...
function tie(element: Element, config: Options): Knot[] {
    ...
    if (config.tagName(elTagName)) {
        level.push({
            name: elTagName,
            penalty: 5,
        })

        const index = indexOf(element, elTagName)
        if (index !== undefined) {
            level.push({
                name: nthOfType(elTagName, index),
                penalty: 10,
            })
        }
    }
...
function indexOf(input: Element, elTagName?: string): number | undefined {
    ...
    while (child) {
        if (
            ...
            (elTagName === undefined || (child as Element).tagName.toLowerCase() === elTagName)
        ) {
            i++
        }
        if (child === input) {
...
function fallback(input: Element, rootDocument: Element | Document | ShadowRoot): Knot[] | undefined {
    let i = 0
    let current: Element | null = input
    const path: Knot[] = []
    while (current && current !== rootDocument) {
        const elTagName = current.tagName.toLowerCase()
        const index = indexOf(current, elTagName)
        if (index === undefined) {
            return
        }
        path.push({
...
function nthChild(elTagName: string, index: number): string {
    if (elTagName === 'html') {
        return 'html'
    }
    return `${elTagName}:nth-child(${index})`
}

function nthOfType(elTagName: string, index: number): string {
    if (elTagName === 'html') {
        return 'html'
    }
    return `${elTagName}:nth-of-type(${index})`
}
...
function unique(path: Knot[], rootDocument: Element | Document | ShadowRoot): boolean {
