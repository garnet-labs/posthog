...
function IdleInput(): JSX.Element {
  ...
  const { setQuery, submitQuery, enterAiMode } =
        ...
        <div className="flex w-full py-1 px-2">
          ...
          <div className="flex items-end shrink-0">
            <div className="flex items-center gap-1 ">
              ...
              <Tooltip
                title={!query.trim() ? "Try asking a question" : undefined}
              >
                <ButtonPrimitive
                  onClick={() => {
                    posthog.capture("homepage query submitted", { mode: "ai" });
...
function HomepageAiInput(): JSX.Element {
  ...
  const { dataProcessingAccepted, dataProcessingApprovalDisabledReason } =
    ...
    return (
      <div className="border border-primary rounded-lg bg-surface-primary p-4 flex flex-col gap-2">
        <p className="font-medium text-pretty m-0">
          PostHog AI needs your approval to potentially process identifying user
          data with external AI providers.
        </p>
...
const GRID_COLUMNS: GridColumn[] = [
  {
    ...
    emptyTooltip:
      'Pin dashboards by clicking "Pin" in the dashboard context panel',
  },
  {
    label: "Recents",
    kind: "recent",
  ...
  },
  {
    ...
    emptyTooltip:
      'Add items to starred by clicking "add to starred" in the context panel',
  },
];
...
function getGridItemContextMenu(
  ...
  actions: {
    ...
    addShortcutItem: (entry: FileSystemEntry) => void;
  },
...
function IdleGrid(): JSX.Element {
  ...
  useEffect(() => {
    const isLoading =
      dashboardsLoading || recentItemsLoading || starredItemsLoading;
    if (isLoading) {
      return;
    }
      ...
      switch (e.key) {
        ...
        case "ArrowRight": {
          ...
          if (nextCol !== null) {
            setHighlight([
              nextCol,
              Math.min(row, columns[nextCol].items.length - 1),
            ]);
          }
          break;
        }
        case "ArrowLeft": {
          ...
          if (prevCol !== null) {
            setHighlight([
              prevCol,
              Math.min(row, columns[prevCol].items.length - 1),
            ]);
          }
          break;
        }
      ...
      <div className="overflow-hidden">
        <div
          ...
          onFocus={(e) => {
            ...
            if (
              !highlight &&
              e.currentTarget.dataset.keyboardFocus === "true"
            ) {
        ...
        >
          {columns.map((col, colIndex) => (
            ...
            >
              ...
              col.items.length === 0 &&
              (skeletonCounts === null ||
                (skeletonCounts[col.kind] ?? 0) > 0) ? (
                Array.from({ length: skeletonCounts?.[col.kind] ?? 3 }).map(
                  (_, i) => (
                    <div key={`skeleton-${i}`}>
...
function HomePageOfframp(): JSX.Element {
  const { showConfigurePinnedTabsModal, showConfigurePinnedTabsTooltip } =
  ...
  return (
    <div className="flex items-center gap-2 mt-8">
      ...
      {previousHomepage && (
        ...
        >
          Put my{" "}
          {(previousHomepage.title || "previous homepage").toLocaleLowerCase()}{" "}
          back <IconRewind className="size-4" />
        </ButtonPrimitive>
      )}
    </div>
  );
